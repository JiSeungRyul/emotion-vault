...
- 사용자 계정/로그인을 추가해 사용자별로 캡슐을 분리하고, 인증되지 않으면 감정 기록/조회 API가 동작하지 않도록 한다.
...
- NFR-003: 접근성—키보드 탐색, 포커스 링 유지, 대비 준수, 폼 필드 레이블/ARIA 제공.
- NFR-004: 보안—비밀번호 해시, 세션/토큰 보호(HTTP-only 쿠키 등), 기본 rate-limit/lockout 고려(추후), 민감 정보 로깅 금지.
- NFR-005: 로깅—생성/목록/상세/인증 성공·실패 모두 `logApiInfo`/`logApiError`로 남긴다, 에러는 코드·메시지·컨텍스트 포함.
